...
namespace InfFlo.Scenes
{
    public class RayTraceToy : IScene
    {
        ...
        public void Draw(RenderComposer composer)
        {
            ...
            var ray = new Ray2D(_rayStart, _rayDirection);
            var barrier = new Line(_barrierStart, _barrierEnd);

            composer.RenderLine(ray.Start, ray.Direction, Color.Green);

            var color = InfFlo.Util.Util.Intersects(ray, barrier, out _crossPoint) ? Color.Red : Color.CornflowerBlue;
